...
try:
    cast(Any, client.collections['hsl']).delete()
...
MISSING_ID_LOG = "log-missing-paragraph-ids.txt"

with open(MISSING_ID_LOG, "w", encoding="utf-8") as log_file:
    log_file.write("")
...
def log_missing_paragraph_id(source: str, paragraph) -> None:
    line_info = paragraph.sourceline or "unknown"
    with open(MISSING_ID_LOG, "a", encoding="utf-8") as log_file:
        log_file.write(f"{source}: paragraph at line {line_info} missing xml:id\n")


def paragraph_anchor(paragraph, html_filename, source_label: str | None = None):
    xml_id = paragraph.get(XML_ID)
    if xml_id:
        return f"{html_filename}#{xml_id}", xml_id
    if source_label:
        log_missing_paragraph_id(source_label, paragraph)
    return None, None
...
def critic_anchor(paragraph, html_filename, source_label: str):
    return paragraph_anchor(paragraph, html_filename, source_label)


def traktat_anchor(paragraph, html_filename, source_label: str):
    return paragraph_anchor(paragraph, html_filename, source_label)


def vms_anchor(paragraph, html_filename, source_label: str):
    return paragraph_anchor(paragraph, html_filename, source_label)


records: list[Dict[str, Any]] = []
cfts_records: list[Dict[str, Any]] = []
for x in tqdm(files, total=len(files)):
    ...
    if 'traktat' in x:
        ...
        for v in facs:
            ...
            cfts_record_template: Dict[str, Any] = {
                'project': 'hsl',
            }
            r_title = " ".join(
                " ".join(doc.any_xpath(
            ...
            for para_idx, para in enumerate(paragraph_nodes, start=1):
                html_id, anchor_id = traktat_anchor(para, html_filename, base_id)
                if not html_id:
                    continue
                record = {
    ...
    if 'critics' in x:
        ...
        for para in paragraphs:
            html_id, anchor_id = critic_anchor(para, html_filename, base_id)
            if not html_id:
                continue
            record = {
    ...
    if 'vms' in x:
        ...
        for para in paragraphs:
            html_id, anchor_id = vms_anchor(para, html_filename, base_id)
            if not html_id:
                continue
            record = {
...
collection = cast(Any, client.collections['hsl'])
make_index = collection.documents.import_(records)
print(make_index)
print('done with indexing hsl')

cfts_collection = cast(Any, CFTS_COLLECTION)
